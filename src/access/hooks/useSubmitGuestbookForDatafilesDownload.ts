...
interface UseSubmitGuestbookForDatafilesDownloadReturn {
  ...
  resetSubmitGuestbookForDatafilesDownloadState: () => void
}

interface UseSubmitGuestbookForDatafilesDownloadProps {
  accessRepository?: AccessRepository
}

export const useSubmitGuestbookForDatafilesDownload = ({
  accessRepository = new AccessJSDataverseRepository()
}: UseSubmitGuestbookForDatafilesDownloadProps = {}): UseSubmitGuestbookForDatafilesDownloadReturn => {
  ...
  const handleSubmitGuestbookForDatafilesDownload = useCallback(
    async (
      fileIds: Array<number | string>,
      answers: GuestbookResponseAnswer[]
    ): Promise<string | undefined> => {
      ...
      try {
        return await submitGuestbookForDatafilesDownload(accessRepository, fileIds, answers)
      } catch (err) {
  ...
  const resetSubmitGuestbookForDatafilesDownloadState = useCallback(() => {
    setIsSubmittingGuestbook(false)
    setErrorSubmitGuestbook(null)
  }, [])
