describe('Dataset', () => {
  ...
  it('successfully loads a dataset metadata with all possible blocks translations', () => {
    ...
    cy.findByText('Metadata').should('exist').click()
    ...
    cy.findByText('Astronomy and Astrophysics Metadata').should('exist')
    cy.findByText('Type').should('exist')

    cy.findByText('Software Metadata (CodeMeta v2.0)').should('exist')
    cy.findByText('Application Category').should('exist')

    cy.findByText('Computational Workflow Metadata').should('exist')
    cy.findByText('Documentation').should('exist')

    cy.findByText('HBGDki Custom Metadata').should('exist')
    cy.findByText('Lower limit of age at enrollment').should('exist')

    cy.findByText('Alliance for Research on Corporate Sustainability Metadata').should('exist')
    cy.findByText(
    ...
    cy.findByText('CHIA Metadata').should('exist')
    cy.findByText('Variables').should('exist')

    cy.findByText('Digaai Metadata').should('exist')
    cy.findByText('Data de Publicação').should('exist')

    cy.findByText('Graduate School of Design Metadata').should('exist')
    cy.findByText('Accreditation').should('exist')

    cy.findByText('MRA Metadata').should('exist')
    cy.findByText('Murray Research Archive Collection').should('exist')

    cy.findByText('PSI Metadata').should('exist')
    cy.findByText('Population').should('exist')

    cy.findByText('Political Science Replication Initiative Metadata').should('exist')
    cy.findByText(
