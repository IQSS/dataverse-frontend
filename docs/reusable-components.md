...
- **Bucket CORS is a deployment requirement for ranged direct download.** With `download-redirect=true` the browser fetches Range parts straight from the object store, so the bucket must allow `GET` from the origin serving the page and expose `Content-Range`, `Accept-Ranges`, `Content-Length` and `ETag`. The block documented in the Dataverse guides (Installation Guide, "Allow CORS for S3 Buckets") already does this: `AllowedOrigins: ["*"]`, `AllowedHeaders: ["*"]`, `AllowedMethods: ["PUT", "GET"]`, `ExposeHeaders: ["ETag", "Accept-Ranges", "Content-Encoding", "Content-Range"]`. A single `bytes=N-M` range is a CORS-safelisted request header, so no preflight is involved; a `bytes=-N` suffix range is not, and the engine never sends one. `HEAD` is not in the documented methods, so size comes from the file metadata rather than a HEAD probe. If a bucket restricts `AllowedOrigins` to the Dataverse host only, a page served from any other origin gets no CORS headers on either the success or the error response, and `fetch` rejects with a bare `TypeError` carrying no status and no body, which the tray reports as an unreadable storage response. The page cannot work around this; the bucket has to be configured. Note that the guides motivate this block with direct upload and previewers, so an installation with direct download enabled and previewers disabled may never have set it.
